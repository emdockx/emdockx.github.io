...
<script>
  kofiWidgetOverlay.draw('stravawattz', {
    'type': 'floating-chat',
...
list(
  ...
  #icon = "hash",
  color = "success",
  value = paste0(length(df_running_365$id), " activities")
)
```
...
list(
  ...
  #icon = "hash",
  color = "warning",
  value = paste0(length(df_cycling_365$id), " activities")
)
```
...
list(
  ...
  #icon = "hash",
  color = "info",
  value = paste0(length(df_swimming_365$id), " activities")
)
```

#### Row {.tabset title="Activities this year" height="25%"}

```{r}
#| padding: 0px
#| title: "2025"
plot_calendar_frontpage_d
```
